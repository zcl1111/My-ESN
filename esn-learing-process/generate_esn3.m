function esn = generate_esn3(nInputUnits, nInternalUnits, nOutputUnits, varargin)
...
esn.noiseLevel = 0.0 ; 
esn.reservoirActivationFunction = 'tanh';%输入激励函数为正切函数
esn.outputActivationFunction = 'identity' ; % options: identity or tanh or sigmoid01输出激励函数为线性
esn.methodWeightCompute = 'pseudoinverse' ; % options: pseudoinverse and wiener_hopf伪逆法和岭回归法
esn.inverseOutputActivationFunction = 'identity' ; 
esn.spectralRadius = 1 ; %谱半径为1
esn.feedbackScaling = zeros(nOutputUnits, 1); %反馈连接为0
esn.trained = 0 ; 
esn.type = 'leaky1_esn' ; 
