...
esn = generate_esn(inSize, resSize,outSize, ...
   'spectralRadius',0.5,'inputScaling',[0.8;0.8],'inputShift',[0;0], ...
    'teacherScaling',[0.8],'teacherShift',[0],'feedbackScaling', 0, ...
    'type', 'plain_esn'); 
...
figure;
%nPlotPoints = 100 ; 
plot(testOutputSequence(7:300),'r-');
hold on;
plot(predictedTestOutput(7:300),'b.');
hold off;
%title('testing: teacher sequence (red) vs predicted sequence (blue)');
title('测试教师数据 (红色实线) vs 预测数据 (蓝色实心点)');
xlim([11,300]);
