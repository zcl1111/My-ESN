...
esn = generate_esn(inSize, resSize,outSize, ...
   'spectralRadius',0.1,'inputScaling',[1;1],'inputShift',[0;0], ...
...
hold on;
scatter3(testInputSequence(:,1),testInputSequence(:,2),testOutputSequence(1+nForgetPoints:testLen,1),'ro');
hold off;
title('测试数据和预测图形');
figure(15);
%训练数据的预测图形
...
hold on;
scatter3(trainInputSequence(:,1),trainInputSequence(:,2),trainOutputSequence(1+nForgetPoints:trainLen,1),'ro');
hold off;
title('训练数据和预测图形');
